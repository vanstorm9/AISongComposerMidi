    ...
    while k < folder_trans.size:
        ...
        for path in os.listdir(folder_trans[k]):
            ...
            if j == 0:
                ...
                tick_u_ar = np.concatenate((tick_u_ar, tick_ar))
                pitch_u_ar = np.concatenate((pitch_u_ar, pitch_ar))
                velocity_u_ar = np.concatenate((velocity_u_ar, velocity_ar))
...
for (sample, target) in ds.getSequenceIterator(0):
    ...
    # Predict and regulate predicted values
